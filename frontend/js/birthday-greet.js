...
async function initBirthdayGreet(containerId) {
    ...
    if (!events.length) {
        el.innerHTML = '<p class="empty-state">No birthdays or anniversaries this week 🎉</p>';
        return;
    }
    ...
    el.innerHTML = `
        ...
        ${events.map(e => {
            const icon = e.eventType === 'birthday' ? '🎂' : '💍';
            ...
            return `
                <div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border)">
                    ...
                    ${e.alreadySent
                        ? '<span style="color:var(--success);font-size:0.82rem">✅ Sent</span>'
                        : `<div style="display:flex;gap:4px">
                            ${e.phone ? `<button class="btn-whatsapp" style="padding:4px 10px;font-size:0.78rem" onclick="sendGreeting('${e.id}','${e.eventType}','whatsapp','${escHtml(e.phone)}','${escHtml(e.name)}')">💬 WhatsApp</button>` : ''}
                            ${e.email ? `<button class="btn-secondary" style="padding:4px 10px;font-size:0.78rem" onclick="sendGreeting('${e.id}','${e.eventType}','email','${escHtml(e.email)}','${escHtml(e.name)}')">📧 Email</button>` : ''}
                            <button class="btn-secondary" style="padding:4px 10px;font-size:0.78rem" onclick="sendGreeting('${e.id}','${e.eventType}','manual','','${escHtml(e.name)}')">✅ Mark Sent</button>
                        </div>`
                    }
                </div>
            `;
...
async function sendGreeting(clientId, type, via, contact, name) {
    ...
    if (via === 'whatsapp' && contact) {
        const msg = type === 'birthday'
            ? `Happy Birthday, ${name}! 🎂🎉 Wishing you a wonderful year ahead. — from your friends at Travel Ops ✈`
            : `Happy Anniversary, ${name}! 💍🥂 Wishing you many more beautiful journeys together. — Travel Ops ✈`;
